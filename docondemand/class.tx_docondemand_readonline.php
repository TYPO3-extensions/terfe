 ...
 *
 * $Id: class.tx_terdochtml_readonline.php 4040 2006-11-09 10:25:56Z bauschan $
 *
...
require_once (t3lib_extMgm::extPath('docondemand').'class.tx_docondemand_api.php');
require_once (t3lib_extMgm::extPath('docondemand').'class.tx_docondemand_documentformat.php');

class tx_docondemand_readonline {
	 ...
	 * @access	public
	 */
	public function render($documentDir) {
		...
		$xsl = new DomDocument();
		$xsl->load(t3lib_extMgm::extPath ('docondemand').'docbook-xsl/xhtml/chunk.xsl');
	 ...
	 * @access	public
	 */
	public function renderDisplay($documentDir, &$pObj) {
		global $TSFE;

		$docApiObj = tx_docondemand_api::getInstance();
		...
		if ($pObj->piVars['html_readonline_chapter'] == 'toc') {
			$content = $this->renderDisplay_renderTOC($tocArr, $documentDir, $pObj);
		} else {
			$csInfoArr = $this->getChapterSectionInformation($pObj, $documentDir);
	 ...
	 * @access	protected
	 */
	protected function renderDisplay_renderTOC($tocArr, $documentDir, &$pObj) {
		global $TSFE;
		...
		$document = str_replace($this->path, '', $documentDir);

		$output = '';
		$docApiObj = tx_docondemand_api::getInstance();

		if (is_array($tocArr)) {
			$TSFE->altPageTitle = 'Documentation: (Table of Contents)';
			$TSFE->indexedDocTitle = $TSFE->altPageTitle;
			...
			foreach ($tocArr as $chapterNr => $chapterArr) {
				...
				$output .= $pObj->pi_linkTP_keepPIvars($docApiObj->csConvHSC($chapterArr['title']), array(
					'html_readonline_chapter' => $chapterNr, 
					'html_readonline_section' => 1,
					'document' => $document
				), 1);
				if (is_array ($chapterArr['sections'])) {
					...
					foreach ($chapterArr['sections'] as $sectionNr => $sectionArr) {
						...
						$output .= $pObj->pi_linkTP_keepPIvars($docApiObj->csConvHSC($sectionArr['title']), 
							array(
								'html_readonline_chapter' => $chapterNr, 
								'html_readonline_section' => $sectionNr, 
								'document' => $document
							), 1);
						if (is_array ($sectionArr['subsections'])) {
							$output .= '<ul>';
							foreach ($sectionArr['subsections'] as $subSectionNr => $subSectionArr) {
								$output .= $this->getChapterSubsectionAnchor($chapterNr, $sectionNr, $subSectionNr, $subSectionArr, $documentDir, $document, $pObj);
							}
							$output .= '</ul>';
						}
						$output .= '</li>';
	 ...
	 * @access	protected
	 */
	protected function getChapterSectionInformation($pObj, $documentDir) {
	...
	public function realurl_renderURLSegementForChapter ($params, $ref) {
		...
		$documentDir = tx_docondemand_api::getInstance()->getDocumentDirOfExtensionVersion ($extensionKey, $version);
		$tocArr = unserialize (@file_get_contents ($documentDir.'toc.dat'));
	 ...
	 * @access	protected
	 */
	protected function getChapterSubsectionAnchor ($chapterNr, $sectionNr, $subSectionNr, $subSectionArr, $documentDir, $document, $pObj) {
		$docApiObj = tx_docondemand_api::getInstance();
		...
		if (preg_match('/(\&\#x201c\;)/',$title))	{
			...
			$out .= preg_replace ('(" >'.$modifiedTitle.')','#'.$anchor[$subSectionNr-1].'" >'.$modifiedTitle.'',$pObj->pi_linkTP_keepPIvars($modifiedTitle, array(
			'html_readonline_chapter' => $chapterNr, 
			'html_readonline_section' => $sectionNr,
			'document' => $document), 1));
			$out .= '</li>';
			...
			$out .= preg_replace ('(" >'.$modifiedTitle.')','#'.$anchor[$subSectionNr-1].'" >'.$docApiObj->csConvHSC($subSectionArr['title']).'',$pObj->pi_linkTP_keepPIvars($docApiObj->csConvHSC($subSectionArr['title']), array(
  'html_readonline_chapter' => $chapterNr, 
  'html_readonline_section' => $sectionNr,
  'document' => $document
  ), 1));
