...
/*******************************************************************
	...
	class Tx_TerFe2_Task_UpdateExtensionListTask extends tx_scheduler_Task {
		...
		/**
		 * TODO: Add additionalFieldProvider for this value
		 *       Maybe also add a checkbox to clean up registry
		 * 
		 * @var integer
		 */
		...
		public function execute() {
			...
			$provider = $this->providerManager->getProvider('extensionmanager');
			$extensions = $provider->getExtensions($lastRun, $offset, $count); // <-- TODO !!!
			...
			*
			...
			$rows = $dataMapper->map('Tx_TerFe2_Domain_Model_Version', array($x));
			* 
			* debug.txt !!!
			*/

				// Set new values to registry
			$registry->add('lastRun', $GLOBALS['EXEC_TIME']);
			$registry->add('offset', $offset + $count);
