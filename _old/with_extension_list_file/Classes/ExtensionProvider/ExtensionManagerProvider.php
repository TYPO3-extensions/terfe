<?php
	...
	class Tx_TerFe2_ExtensionProvider_ExtensionManagerProvider extends Tx_TerFe2_ExtensionProvider_AbstractProvider {
		...
		public function getExtensions($lastRun, $offset, $count) {
				// Get extension list
			if (!empty($this->configuration['useExtensionListFile'])) {
				$filename = PATH_site . $this->extensionListFile;
				$extensions = $this->getExtensionsFromFile($filename, $lastRun);
			} else {
				$extensions = $this->getExtensionsFromRepository($lastRun);
			}
			if (empty($extensions)) {
				return array();
			}
			...
			t3lib_div::writeFile(PATH_site . 'debug.txt', print_r($extensions, TRUE));
		}
		 ...
		 */
		protected function getExtensionsFromFile($filename, $lastRun = 0) {
			if (empty($filename) || !Tx_TerFe2_Utility_Files::fileExists($filename)) {
		...
		/**
		 * Get updated extensions from repository
		 * 
		 * @param integer $lastRun Timestamp of last update
		 * @return array All extension information
		 */
		protected function getExtensionsFromRepository($lastRun = 0) {
			$extensionRows = $this->extensionRepository->findLastUpdated($lastRun);
