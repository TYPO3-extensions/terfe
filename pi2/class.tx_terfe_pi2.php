...
class tx_terfe_pi2 extends tslib_pibase {
	...
	protected function init($conf) {
		...
		if (substr($this->commonObj->repositoryDir, -1, 1) != '/') {
			$this->commonObj->repositoryDir .= '/';
		}
		$this->commonObj->init();
	}
	...
	public function main($content, $conf) {
		...
		$this->init($conf);
		if (!@is_dir($this->commonObj->repositoryDir)) {
			return 'TER_FE Error: Repository directory (' . $this->commonObj->repositoryDir . ') does not exist!';
		}
	...
	protected function renderView_registerKeys() {
		...
		switch (t3lib_div::_GP('tx_terfe_pi2_cmd')) {

			case 'registersubmit':
				$extensionKey = $TSFE->csConv(t3lib_div::_GP('tx_terfe_pi2_extensionkey'), 'utf-8');

				if (!strlen(t3lib_div::_GP('tx_terfe_pi2_extensiontitle'))) {
					$output .= '
					...
					try {
						$result = $soapClientObj->checkExtensionKey($this->accountData, $extensionKey);
						// remove cookies   * no more needed as there is a new client created
						...
						if (strcmp(TX_TER_RESULT_EXTENSIONKEYDOESNOTEXIST, $result['resultCode']) == 0) {
							$extensionKeyDataArr = array(
								...
								'title' => $TSFE->csConv(t3lib_div::_GP('tx_terfe_pi2_extensiontitle'), 'utf-8'),
								'description' => $TSFE->csConv(t3lib_div::_GP('tx_terfe_pi2_extensiondescription'), 'utf-8'),
							);
							$soapClientObj = $this->getSoapClient();
							$result = $soapClientObj->registerExtensionKey($this->accountData, $extensionKeyDataArr);
			...
			switch (t3lib_div::_GP('tx_terfe_pi2_cmd')) {
					...
					if (is_array($resultArr)) {
						switch ($resultArr['resultCode']) {
							case TX_TER_RESULT_GENERAL_OK :
								$actionMessages = '<p>' . $iconInfo . ' <strong>' . sprintf($this->pi_getLL('managekeys_action_transferkey_success', '', 1), $extensionKey, $targetUsername) . '</strong></p><br />';
								break;
							case TX_TER_ERROR_GENERAL_USERNOTFOUND:
								$actionMessages = '<p>' . $iconError . ' <strong>' . sprintf($this->pi_getLL('managekeys_action_transferkey_usernotfound', '', 1), $extensionKey, $targetUsername) . '</strong></p><br />';
								break;
							default:
								$actionMessages = '<p>' . $iconError . ' <strong>' . sprintf($this->pi_getLL('general_errorcode', '', 1), $resultArr['resultCode']) . '</strong></p><br />';
								break;
						}
					}
					break;
				...
				case 'deletekey':
					...
					if (is_array($resultArr)) {
						switch ($resultArr['resultCode']) {
							case TX_TER_RESULT_GENERAL_OK :
								$actionMessages = '<p>' . $iconInfo . ' <strong>' . sprintf($this->pi_getLL('managekeys_action_deletekey_success', '', 1), $extensionKey) . '</strong></p><br />';
								break;
							default:
								$actionMessages = '<p>' . $iconError . ' <strong>' . sprintf($this->pi_getLL('general_errorcode', '', 1), $resultArr['resultCode']) . '</strong></p><br />';
								break;
