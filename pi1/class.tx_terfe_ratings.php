 ...
 *  Copyright notice
 *
 *  (c) 2006 Michael Scharkow <michael@underused.org>
 *  All rights reserved
 *
...
/**
 * Plugin 'Ratings' for the 'ter_fe' extension.
 *
 * $Id$
 *
 * @package TYPO3
...
class tx_terfe_ratings
{
	...
	function renderSingleView_rating()
	{
		...
		if ($history) {
			$output .= '<li class="terfe-rating-history">' . $history . '</li>';
		}

		// Rendering form
		if ($this->canBeRated()) {
			$output .= '<li class="terfe-rating-form">' . $this->render_ratingForm() . '</li>';
		}
		...
		if (is_array($ratings[0])) {
			...
			foreach ($ratings as $rating) {
				...
				$output .= '
					...
					<td>' . ($rating['funcrating'] > 0 ? $rating['funcrating'] : '-') . '</td>
					<td>' . ($rating['docrating'] > 0 ? $rating['docrating'] : '-') . '</td>
					<td>' . ($rating['coderating'] > 0 ? $rating['coderating'] : '-') . '</td>
	...
	private function db_getAvgRating($extensionkey, $version, $previous = FALSE)
	{
		...
		$res = $TYPO3_DB->exec_SELECTquery(
			...
			. ($previous ? ' AND version <= '
					: ' AND version =') . $TYPO3_DB->fullQuoteStr($version, 'tx_terfe_ratings')
		);
		if ($res) {
